...
const App: React.FC = () => {
  ...
  // Bezpieczny dostęp do klucza (Vite powinien go zamienić podczas buildu)
  const getApiKey = () => {
    try {
  ...
  const processFile = async (file: File) => {
    if (!isKeyValid) {
      setErrorMessage("Błąd krytyczny: Brak klucza API w kodzie aplikacji. System nie może połączyć się z AI.");
      return;
    }
      ...
      <div className="min-h-screen bg-yellow-400 flex items-center justify-center p-6 font-mono border-[16px] border-black text-black">
        <div className="max-w-2xl w-full bg-black text-white p-12 shadow-[20px_20px_0px_0px_rgba(0,0,0,0.3)]">
          <div className="border-b-4 border-yellow-400 pb-6 mb-8">
            <h1 className="text-5xl font-black uppercase tracking-tighter italic">AutoMeasure <span className="text-yellow-400">V4.7</span></h1>
            <p className="text-yellow-400 font-bold mt-2 tracking-widest uppercase">CBM Polska Laboratory System</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            {!isKeyValid && (
              <div className="text-[10px] text-red-400 uppercase font-bold text-center leading-relaxed">
                UWAGA: System nie wykrył klucza API podczas budowania aplikacji. <br/>
                Wykonaj REDEPLOY na Vercel z wyczyszczonym cache (Clear Cache).
              </div>
            )}
          </div>
        </div>
    ...
    <div className="min-h-screen bg-zinc-100 flex flex-col font-mono text-black">
      <header className="bg-black text-white p-6 flex justify-between items-center border-b-8 border-yellow-400">
        <div className="flex items-center gap-4">
           <div className="bg-yellow-400 text-black px-3 py-1 font-black">V4.7</div>
           <h1 className="font-black uppercase italic text-xl tracking-tighter">AutoMeasure <span className="text-yellow-400">CBM</span></h1>
        </div>
        <button 
        ...
        {!drawingData ? (
          <div className="mt-20 flex flex-col items-center">
            ...
            >
              ...
              <h2 className="text-3xl font-black uppercase mb-4 tracking-tighter italic">Załaduj Rysunek</h2>
          ...
          <div className="grid lg:grid-cols-2 gap-12 animate-in fade-in duration-500">
             ...
             <div className="bg-white border-8 border-black p-8 shadow-[10px_10px_0px_0px_rgba(250,204,21,1)]">
                <div className="flex justify-between items-start mb-8 border-b-4 border-zinc-100 pb-6">
                   <div>
                      <h2 className="text-3xl font-black uppercase tracking-tighter">{drawingData.partName || "Część CBM"}</h2>
      ...
      {state === AppState.ANALYZING && (
        <div className="fixed inset-0 bg-yellow-400 z-[200] flex flex-col items-center justify-center border-[20px] border-black text-center p-10 animate-in fade-in duration-300">
          ...
          <h2 className="text-6xl font-black uppercase italic tracking-tighter mb-4">Skanowanie...</h2>
